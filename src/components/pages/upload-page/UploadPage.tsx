...
import { deleteFileRequest, getListFiles, uploadFileRequest } from '../../../domains/upload/upload.services';
...
export const UploadPage = () => {
  ...
  const getFilesInfoHandler = async () => {
    ...
    if (error)
      console.log('[GetFilesInfo] [UploadPage] Response::', JSON.stringify(error));
    if (data) setData(data);
  };

  const preDeleteHandler = (dat: any) => {
    const exist = (dat.includes('Mandibular') | dat.includes('Maxillary')) ? dat.replace(`./public/uploads/${user}/`, '')
      : undefined;
    console.log({ exist })
